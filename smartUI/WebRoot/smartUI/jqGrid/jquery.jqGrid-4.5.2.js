...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: {page:"page",rows:"rows", sort: "sidx",order: "sord", search:"search", nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
			...
			ignoreCase : false,
			cmTemplate : {title:true}, // 不要提示tooltip
			idPrefix : "",
			multiSort :  false
		}, $.jgrid.defaults, pin || {});
		var ts= this, grid={
					...
					$(ts.rows).each(function(i) {
						if (i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled')){
								// zys修改：全选时不选中checkbox disabled=true 的
								var jqgcbox = $("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id) );
								if(!jqgcbox[ts.p.useProp ? 'prop': 'attr']("disabled")){
									jqgcbox[ts.p.useProp ? 'prop': 'attr']("checked",true);
		...
		$(grid.cDiv).append("<table class='ui-jqgrid-keyanBar'><tr><td class='ui-jqgrid-keyanBar-home'><span class='ui-jqgrid-title'>"+ts.p.caption+"</span></td></tr></table>")
		...
		*/
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			ruleFieldSelect.bind('change',function() {
				...
				//zys:var elm = $.jgrid.createEl.call($t, cm.inputtype,cm.searchoptions, "", true, that.p.ajaxSelectOptions, true);
			...
			//zys: var ruleDataInput = $.jgrid.createEl.call($t, cm.inputtype,cm.searchoptions, rule.data, true, that.p.ajaxSelectOptions, true);
			...
			if(this.p.ruleButtons === true) {
			var ruleDeleteInput = $("<input type='button' style='margin-right:10px;' value='-' title='Delete rule' class='delete-rule ui-del'/>");
...
var rp_ge = {};
$.jgrid.extend({
	setGridPostData : function (pd, refresh) { // postData添加参数
		var postData = this.getGridParam('postData');
		$.extend(postData, pd);
		if(refresh){
			this.jqGrid("setGridParam", {search:false}).trigger("reloadGrid", [{page:1}]);
		}
	},
	fuzzysGrid : function () { // 模糊查询
		var postData = this.getGridParam('postData');
		if(postData){
			if(postData.filters) delete postData.filters;
			if(postData.searchField) delete postData.searchField;
			if(postData.searchString) delete postData.searchString;
			if(postData.searchOper) delete postData.searchOper;
		}
		var gview_div = $("#gview_"+$(this).attr("id"));
		var text = $.trim($('.ui-jqgrid-keyanBar-sch1-input',gview_div).val());
		var pros = new Array();
		var colms = this.getGridParam('colModel');
		for (var i = 0; i < colms.length; i++) {
			// but show only serchable and serchhidden = true fields
			if(colms[i].searchoptions){
				if(colms[i].searchoptions.fuzzys){
					pros.push(colms[i].index);
				}
			}
		}
		var fuzzy = {'fuzzyText':text, 'fuzzyPros':pros.join(',')};
		$.extend(postData, fuzzy);
		this.jqGrid("setGridParam", {search:false,sortorder:"desc"}).trigger("reloadGrid", [{page:1}]);
	},
	searchGrid : function (p) {
		var postData = this.getGridParam('postData');
		if(postData.fuzzyText) delete postData.fuzzyText;
		if(postData.fuzzyPros) delete postData.fuzzyPros;
		p = $.extend(true, {
		...
		return this.each(function() {
			...
			function showFilter(_filter) {
				...
				$(".searchFilter select:first").select();
			}
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				var gbox_div = $("#gbox_"+$t.p.id);
				if(o.keyanSelect){
					var keyanBar = $(".ui-jqgrid-keyanBar",gbox_div);
					tbd = $("<td class='ui-jqgrid-keyanBar-keyanSelect'></td>").append($(o.keyanSelect));
					var sel = $("select:first",tbd);
					$(sel).change(function(){
						var selName = $(this).attr("name");
						var selVal = $(this).val();
						var osel = {};
						osel[selName] = selVal;
						$($t).setGridPostData(osel,true);
					});
					$("tr",keyanBar).append(tbd);
					$(o.keyanSelect).show(); // 如果display:none
					tbd = null;
				}
				if(o.keyanSch1){
					var keyanBar = $(".ui-jqgrid-keyanBar",gbox_div);
					tbd = $("<td class='ui-jqgrid-keyanBar-sch1-td1'><span class='ui-jqgrid-keyanBar-sch1-span1'><input type='text' value='' class='ui-jqgrid-keyanBar-sch1-input'/><span class='ui-jqgrid-keyanBar-sch1-span2'></span></span></td>");
					$("tr",keyanBar).append(tbd);
					tbd = $("<td class='ui-jqgrid-keyanBar-sch1-td2'><div><span></span>查找</div></td>");
					$("tr",keyanBar).append(tbd);
					$(tbd,keyanBar).attr({"title":"模糊查询"})
					.click(function(){
						$($t).jqGrid("fuzzysGrid",{});
						return false;
					}).hover(onHoverIn, onHoverOut);
					$(".ui-jqgrid-keyanBar-sch1-span2").click(function(){
						$(this).prev("input").val("");
						$($t).jqGrid("fuzzysGrid",{});
					});
					tbd = null;
				...
				if(o.keyanSch2){
					var keyanBar = $(".ui-jqgrid-keyanBar",gbox_div);
					tbd = $("<td class='ui-jqgrid-keyanBar-fplus'></td>");
					$(tbd).append("<div><span></span>筛选</div>");
					$("tr",keyanBar).append(tbd);
					$(tbd,keyanBar).attr({"title":"按条件筛选数据"})
					.click(function(){
						if($(".ui-jqgrid-keyanBar-fplus", keyanBar).length){
							$($t).jqGrid("filterToolbar", {searchOperators : true});
							$(".ui-jqgrid-keyanBar-fplus", keyanBar).addClass("ui-jqgrid-keyanBar-fminus").removeClass("ui-jqgrid-keyanBar-fplus");
						} else {
							$($t).jqGrid("destroyFilterToolbar");
							$(".ui-jqgrid-keyanBar-fminus", keyanBar).addClass("ui-jqgrid-keyanBar-fplus").removeClass("ui-jqgrid-keyanBar-fminus");
						}
						return false;
					}).hover(onHoverIn, onHoverOut);
					tbd = null;
				...
				if(o.keyanSch3){
					var keyanBar = $(".ui-jqgrid-keyanBar",gbox_div);
					tbd = $("<td class='ui-jqgrid-keyanBar-sch2'></td>");
					$(tbd).append("<div><span></span>高级查询</div>");
					$("tr",keyanBar).append(tbd);
					$(tbd,keyanBar).attr({"title":"多条件组合精确查询"})
					.click(function(){
						$($t).jqGrid("searchGrid",{});
						return false;
					}).hover(onHoverIn, onHoverOut);
				...
				if(o.keyanAdd){
					$($t).jqGrid('keyanBtnAdd',{
						caption : "添加",
						title : "添加",
						buttonicon : "ui-jqgrid-keyanBar-add",
						onClickButton : o.keyanAdd
					});
				}
				if(o.keyanDel){
					$($t).jqGrid('keyanBtnAdd',{
						caption : "删除",
						title : "删除",
						buttonicon : "ui-jqgrid-keyanBar-del",
						onClickButton : o.keyanDel
					});
				}
				...
				if (o.setCols) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.setColsicon+"'></span>"+o.setColstext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.setColstitle  || "",id: "refresh_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							//$($t).columnChooser();
							$($t).setColumns();
	...
	keyanBtnAdd : function (p) { // keyan
		...
		return this.each(function() {
			var $t = this;
			var gbox_div = $("#gbox_"+$t.p.id);
			var findnav = $(".ui-jqgrid-keyanBar",gbox_div);
			if (findnav) {
				var tbd = $("<td></td>");
				$(tbd).addClass('ui-jqgrid-keyanBar-td '+p.buttonicon+'').append("<div><span></span>"+p.caption+"</div>");
				$("tr",findnav).append(tbd);
				$(tbd,findnav).attr("title",p.title  || "")
				.click(function(e){
					if (!$(this).hasClass('ui-state-disabled')) {
						if ($.isFunction(p.onClickButton) ) {p.onClickButton.call($t,e);}
					}
...
$.jgrid.extend({
    ...
    columnChooser : function(opts) {
        ...
        var selector = $('<div id="colchooser_'+self[0].p.id+'" style="position:relative;overflow:hidden;padding: 2px 0px;"><div><select multiple="multiple"></select></div></div>');
